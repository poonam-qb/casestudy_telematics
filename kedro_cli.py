...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/casestudy_telematics_pkg")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=casestudy_telematics_pkg"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Car_telematics_casestudy"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "casestudy_telematics_pkg"
